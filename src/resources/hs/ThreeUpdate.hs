...
-- Interfaz:
createUpdateFunction :: (Double -> ThreeAnimation ()) -> (Double -> JSString)
createUpdateFunction f = (\frame -> let list = batchTransformations (Prelude.reverse (Prelude.snd (runThreeAnimation (f frame)))) in (encodeJSON (listToJson list)))
